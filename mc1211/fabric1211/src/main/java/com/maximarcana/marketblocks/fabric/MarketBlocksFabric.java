...
/**
 * Fabric 1.21.1 entrypoint for Market Blocks v1.0.6.
 *
 * <p>1.21.1 notes vs the 26.x build: item models in
 * {@code models/item/*.json} are picked up automatically (no registration
 * needed on Fabric); break protection uses
 * {@code PlayerBlockBreakEvents.BEFORE} instead of the 26.x mixin.
 */
public final class MarketBlocksFabric implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        // 1.21.1: creative-tab events live in the itemgroup package
        // (creativetab.v1 does not exist in this Fabric API version).
        ItemGroupEvents.modifyEntriesEvent(ResourceKey.create(Registries.CREATIVE_MODE_TAB,
                ResourceLocation.fromNamespaceAndPath("minecraft", "functional_blocks")))
            .register(output -> {
                // 1.21.1: call the vanilla CreativeModeTab.Output method
                // directly. Fabric's FabricItemGroupEntries.accept overloads
                // vary between Fabric API versions (caused a
                // NoSuchMethodError in the wild); the vanilla 2-arg accept
                // is stable on every version.
