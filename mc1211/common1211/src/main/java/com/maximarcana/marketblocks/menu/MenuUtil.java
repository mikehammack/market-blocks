...
public final class MenuUtil {
    ...
    /**
     * Adds the standard 27-slot main inventory + 9-slot hotbar for the player.
     * 1.21.1 note: this was an {@code AbstractContainerMenu} method in 26.x,
     * and {@code addSlot} is protected in 1.21.1, so menus pass
     * {@code this::addSlot} as the adder.
     */
    public static void addStandardInventorySlots(
            java.util.function.Consumer<Slot> adder,
            net.minecraft.world.entity.player.Inventory playerInventory, int x, int y) {
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                adder.accept(new Slot(playerInventory, col + row * 9 + 9, x + col * 18, y + row * 18));
            }
        }
        for (int col = 0; col < 9; col++) {
            adder.accept(new Slot(playerInventory, col, x + col * 18, y + 58));
        }
    }
    ...
    public static int giveOrDrop(ServerPlayer player, ItemStack stack) {
        ...
        if (dropped > 0) {
            player.drop(stack.copyWithCount(dropped), false);
        }
        return toGive;
    }
}
