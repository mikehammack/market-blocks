 ...
 * re-reading NBT every click. Supports .schematic (Schematica) and
 * .schem (Sponge).
 */
public final class SchematicLibrary {
    ...
    public static File folder() {
        ...
        if (!dir.isAbsolute()) {
            ...
            if (gameDir != null) {
                dir = gameDir.resolve(raw).toFile();
            }
        }
        if (!dir.exists()) {
            dir.mkdirs();
    ...
    private static Path gameDir() {
        try {
            Path configFile = MarketConfig.configPath();
            if (configFile != null) {
                // config/marketblocks.toml -> game dir
                Path configDir = configFile.getParent();
                if (configDir != null && configDir.getFileName().toString().equals("config")) {
    ...
    /** All .schematic and .schem files in the folder, sorted by name. */
    public static List<File> scanFiles() {
        File dir = folder();
        File[] files = dir.listFiles((d, name) -> {
            String lower = name.toLowerCase();
            return lower.endsWith(".schematic") || lower.endsWith(".schem");
        });
        if (files == null) {
